...
- Rename `***REMOVED***u***REMOVED***-max-content` and `***REMOVED***u***REMOVED***-min-content` utilities to `***REMOVED***u***REMOVED***-max` and `***REMOVED***u***REMOVED***-min` in experimental extended spacing scale ([#2532](https://github.com/tailwindlabs/tailwindcss/pull/2532))
...
- Fix bug where the new `applyComplexClasses` experiment didn't behave as expected with rules with multiple selectors, like `.foo, .bar ***REMOVED*** color: red ***REMOVED***`
...
- Fix bug where target: 'browserslist' didn't work, only `target: ['browserslist', ***REMOVED***...***REMOVED***]` did
...
- Fix issue where `purge: ***REMOVED*** enabled: false ***REMOVED***` was ignored, add `purge: false` shorthand
...
- Fix bug where `divide-***REMOVED***x/y***REMOVED***-0` utilities didn't work due to missing unit in `calc` call
...
- CSS Grid support
- Added `max-w-***REMOVED***screen***REMOVED***` utilities
...
- CSS Transform support
- CSS Grid support
- New `max-w-***REMOVED***screen***REMOVED***` utilities
- Added `max-w-none` utility
