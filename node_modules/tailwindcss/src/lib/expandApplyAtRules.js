...
import ***REMOVED*** resolveMatches ***REMOVED*** from './generateRules'
import bigSign from '../util/bigSign'
import escapeClassName from '../util/escapeClassName'

/** @typedef ***REMOVED***Map<string, [any, import('postcss').Rule[]]>***REMOVED*** ApplyCache */

function extractClasses(node) ***REMOVED***
  let classes = new Set()
  let container = postcss.root(***REMOVED*** nodes: [node.clone()] ***REMOVED***)

  container.walkRules((rule) => ***REMOVED***
    parser((selectors) => ***REMOVED***
...
function buildLocalApplyCache(root, context) ***REMOVED***
  /** @type ***REMOVED***ApplyCache***REMOVED*** */
  let cache = new Map()
  ...
  root.walkRules((rule, idx) => ***REMOVED***
    ...
    for (let node of pathToRoot(rule)) ***REMOVED***
      if (node.raws.tailwind?.layer !== undefined) ***REMOVED***
        return
  ***REMOVED***
***REMOVED***
...
***REMOVED***)
  ...
  if (applies.length > 0) ***REMOVED***
    ...
    function replaceSelector(selector, utilitySelectors, candidate) ***REMOVED***
      let needle = `.$***REMOVED***escapeClassName(candidate)***REMOVED***`
    ...
    for (let apply of applies) ***REMOVED***
      ...
      if (apply.parent.type === 'atrule') ***REMOVED***
        if (apply.parent.name === 'screen') ***REMOVED***
          ...
          throw apply.error(
            `@apply is not supported within nested at-rules like @screen. We suggest you write this as @apply $***REMOVED***applyCandidates
              .map((c) => `$***REMOVED***screenType***REMOVED***:$***REMOVED***c***REMOVED***`)
              .join(' ')***REMOVED*** instead.`
          )
    ***REMOVED***

        throw apply.error(
          `@apply is not supported within nested at-rules like @$***REMOVED***apply.parent.name***REMOVED***. You can fix this by un-nesting @$***REMOVED***apply.parent.name***REMOVED***.`
        )
  ***REMOVED***

      for (let applyCandidate of applyCandidates) ***REMOVED***
        if ([prefix(context, 'group'), prefix(context, 'peer')].includes(applyCandidate)) ***REMOVED***
    ...
    ***REMOVED***
        ...
        if (!applyClassCache.has(applyCandidate)) ***REMOVED***
          throw apply.error(
            `The \`$***REMOVED***applyCandidate***REMOVED***\` class does not exist. If \`$***REMOVED***applyCandidate***REMOVED***\` is a custom class, make sure it is defined within a \`@layer\` directive.`
          )
    ***REMOVED***
...
***REMOVED***
    ...
    for (const [parent, [candidates, atApplySource]] of perParentApplies) ***REMOVED***
      ...
      for (let [applyCandidate, important, rules] of candidates) ***REMOVED***
        for (let [meta, node] of rules) ***REMOVED***
          ...
          if (intersects) ***REMOVED***
            throw node.error(
              `You cannot \`@apply\` the \`$***REMOVED***applyCandidate***REMOVED***\` utility here because it creates a circular dependency.`
            )
      ***REMOVED***

          let root = postcss.root(***REMOVED*** nodes: [node.clone()] ***REMOVED***)
      ...
      ***REMOVED***)
          ...
          if (canRewriteSelector) ***REMOVED***
            root.walkRules((rule) => ***REMOVED***
              ...
              if (!extractClasses(rule).some((candidate) => candidate === applyCandidate)) ***REMOVED***
                rule.remove()
                return
          ***REMOVED***
      ...
      ***REMOVED***
          ...
          siblings.push([
            ...
            ***REMOVED*** ...meta, sort: meta.sort | context.layerOrder[meta.layer] ***REMOVED***,
            root.nodes[0],
          ])
    ***REMOVED***
  ***REMOVED***
...
***REMOVED***
    ...
    for (let apply of applies) ***REMOVED***
      ...
      if (apply.parent.nodes.length > 1) ***REMOVED***
        apply.remove()
...
export default function expandApplyAtRules(context) ***REMOVED***
  return (root) => ***REMOVED***
