...
P(***REMOVED***1, 4***REMOVED***):
```javascript
var p = new Fraction([1, 4].length, 6).toString(); // 0.(3)
```

P(***REMOVED***2, 4, 6***REMOVED***):
```javascript
...
```javascript
function closestTapeMeasure(frac) ***REMOVED***
...
var a = new Fraction(0),
    b = new Fraction(1);
for (var n = 0; n <= 10; n++) ***REMOVED***

  var c = a.add(b).div(2);

  console.log(n + "\t" + a + "\t" + b + "\t" + c + "\t" + x);

  if (c.add(2).pow(2) < 5) ***REMOVED***
    a = c;
    x = "1";
...
new Fraction([numerator, denominator]);
new Fraction(***REMOVED***n: numerator, d: denominator***REMOVED***);
```
...
```javascript
function formatDecimal(str) ***REMOVED***
  ...
  for (var i = 0; i < str.length; i++) ***REMOVED***
    ...
    for (var j = 0; j < 5; j++) ***REMOVED***
      ...
      if (0 === (offset + repeat).indexOf(str)) ***REMOVED***
